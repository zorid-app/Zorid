import { DisposableStack, ZoridError, type Disposable, type JsonValue } from '@zorid/shared';
import { apiInfoFixture, type AppAPI, type CapabilityName, type CommandContribution, type EventBusAPI, type SettingsContribution } from '@zorid/platform-api';
...
export class EventBus implements EventBusAPI {
  ...
  on<T = unknown>(event: string, listener: (payload: T) => void): Disposable {
    ...
    return { dispose: () => { listeners.delete(listener as (payload: unknown) => void); } };
  }
...
export class CommandRegistry {
  ...
  register(command: CommandContribution): Disposable {
    if (this.#commands.has(command.id)) throw new ZoridError('command.duplicate', `Command already registered: ${command.id}`);
...
export class SettingsRegistry {
  ...
  register(section: SettingsContribution): Disposable {
    if (this.#settings.has(section.id)) throw new ZoridError('settings.duplicate', `Settings section already registered: ${section.id}`);
    this.#settings.set(section.id, section);
    return { dispose: () => { this.#settings.delete(section.id); } };
  }
...
export class CapabilityRegistry {
  ...
  requireAll(capabilities: readonly CapabilityName[], pluginId?: string): readonly CapabilityName[] {
    ...
    for (const capability of missing) this.#diagnostics.push(pluginId === undefined ? { code: 'plugin.capability.missing', capability } : { code: 'plugin.capability.missing', capability, pluginId });
    return missing;
  }
...
export function createZoridKernel(options: { version?: string; capabilities?: Iterable<CapabilityName> } = {}): ZoridKernel {
  return {
