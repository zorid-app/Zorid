...
import {
  ...
  parseZbase,
  parseZtype,
} from '../packages/object-store/src/index';
...
function fakeContainer(): HTMLElement {
  ...
  } as unknown as HTMLElement & { children: Array<{ textContent: string | null }> };
  Object.defineProperty(node, 'textContent', {
    get: () => children.map((child) => child.textContent ?? '').join(''),
    set: (value: string | null) => {
      children.splice(0);
      if (value) children.push({ textContent: value });
    },
  });
  ...
  it('exposes host-owned FieldsAPI and preserves ad-hoc fields across type changes', async () => {
    ...
    const [record] = (
      await engine.indexFiles({
        files: [
          {
            path: normalizeVaultPath('Task.md'),
            contents: `---
zorid:
  type: task
status: open
...
# Task`,
    ...
    await expect(fields.getFields(normalizeVaultPath('Task.md'))).resolves.toContainEqual({
      key: 'custom',
      value: 'keep',
      source: 'frontmatter',
    });
  ...
  it('evaluates .zbase filters and renders embeds with host-derived caller identity context', async () => {
    ...
    const objects = createObjectStoreService({
      '.zorid/views/tasks.zbase': JSON.stringify({
        views: [{ id: 'open', renderer: 'table', filters: { expression: { equals: ['status', 'open'] } } }],
      }),
    });
    ...
    await expect(dataViews.evaluateFilters({ expression: { equals: ['status', 'open'] } })).resolves.toMatchObject([
      { path: 'A.md' },
    ]);
    const container = fakeContainer();
    const disposable = await dataViews.renderEmbed(container, normalizeVaultPath('.zorid/views/tasks.zbase'), {
      basePath: normalizeVaultPath('.zorid/views/tasks.zbase'),
    });
