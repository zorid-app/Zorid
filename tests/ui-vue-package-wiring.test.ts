...
describe('ui-vue package wiring', () => {
  ...
  it('lets the desktop renderer consume shared UI sources and tokens through workspace aliases', () => {
    ...
    const rootPkg = readJson<{ devDependencies?: Record<string, string>; scripts?: Record<string, string> }>(
      'package.json',
    );
    ...
    expect(electronConfig).toContain(
    ...
    expect(baseTsconfig.compilerOptions?.paths?.['@zorid/ui-vue/tokens.css']).toEqual([
      'packages/ui-vue/src/tokens.css',
    ]);
    expect(baseTsconfig.compilerOptions?.paths?.['@zorid/ui-vue/components.css']).toEqual([
