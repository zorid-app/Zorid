...
describe('index/metadata/search slice', () => {
  ...
  it('rebuilds Markdown records and exposes metadata/search APIs', async () => {
    ...
    await expect(metadata.getFile(normalizeVaultPath('A.md'))).resolves.toMatchObject({ headings: ['Title'], tags: ['project','active','inline'] });
  ...
  it('supports incremental updates by upserting changed records', async () => {
    const engine = createJsIndexEngine();
    const store = new InMemoryIndexStore();
    const [initial] = (await engine.indexFiles({ files: [{ path: normalizeVaultPath('A.md'), contents: '# Old' }] })).records;
    if (!initial) throw new Error('missing initial record');
    store.upsert(initial);
    const [changed] = (await engine.indexFiles({ files: [{ path: normalizeVaultPath('A.md'), contents: '# New\n#tag' }] })).records;
